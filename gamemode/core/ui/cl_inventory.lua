...
function PANEL:SetInfo(id)
    ...
    name:SetFont("ax.fonts.extralarge.bold")
    name:SetText(item:GetName(), true)

    local description = item:GetDescription()
    local descriptionWrapped = ax.util:GetWrappedText(description, "ax.fonts", self.info:GetWide() - 32)
    for k, v in pairs(descriptionWrapped) do
        local text = self.info:Add("ax.text")
        text:Dock(TOP)
        text:SetFont("ax.fonts")
        text:SetText(v, true)
    end
end
...
function PANEL:Init()
    ...
    self.name:Dock(FILL)
    self.name:SetFont("ax.fonts.large")
